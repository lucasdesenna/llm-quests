...
3. Resolve a quest id with `bash "/absolute/path/to/quest-state/scripts/quest-resolve.sh" "/absolute/path/to/quests" <query>`. If no query was provided, run the quest-list skill with the same `quests-dir`. If script resolution is ambiguous, show the candidates and stop.
...
5. Validate structure with `bash "/absolute/path/to/quest-state/scripts/quest-validate.sh" "{quests-dir}" "{quest-id}"` before acting on the quest.
...
7. Apply final phase transitions only from the main session after reviewing phase output. Check `references/lifecycle.md`, then run `bash "/absolute/path/to/quest-state/scripts/quest-transition.sh" <from> <to>` for forward transitions.
